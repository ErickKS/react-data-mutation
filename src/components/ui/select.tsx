  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        sideOffset={6}
        position="popper"
        className={twMerge("z-50 text-sm max-h-96 min-w-[6rem] overflow-hidden rounded-md border border-zinc-800 bg-zinc-900", className)}
        {...props}
      />
    </SelectPrimitive.Portal>
  );
...
export function SelectItem({ className, children, ...props }: SelectItemProps) {
  return (
    <SelectPrimitive.Item
      className={twMerge("flex items-center gap-2 text-zinc-300 px-3 py-1.5 justify-between outline-none hover:bg-zinc-800", className)}
      {...props}
    >
